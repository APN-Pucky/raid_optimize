use crate::{
    data::{
        effect::Effect,
        skill::{Select, SkillType},
    },
    wave::heroes::{Cooldown, Execute, Selector, Skilled, Typed},
};
...
impl Execute for SoulSeal {
    fn execute(
        ...
        wave.attack_single(
            actor,
            target,
            wave.get_attack_damage(actor)
                * (self.attack_damage_ratio + poison as f32 * self.attack_damage_ratio_per_poison),
            skill,
        );
