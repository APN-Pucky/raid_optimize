    ...
    pub fn execute(&self, wave : &mut Wave,  skill : &Skill, actor:InstanceIndex, target:InstanceIndex, ) {
                ...
                for t in wave.get_ally_indices(actor) {
                    if wave.health[t] == wave.get_max_health(t) {
                        wave.inflict_single(actor, t, Effect::ConsolidationI, 1.0, self.consolidation_turns);
                    }
                    wave.restore_single(actor, t, self.max_hp_restore_ratio* wave.get_max_health(actor));
                }
                // 2nd
                for t in wave.get_ally_indices(actor) {
                    if wave.health[t] == wave.get_max_health(t) {
                        wave.shield_single(actor, t, wave.get_max_health(actor) * self.shield_max_hp_ratio, self.shield_turns);
                    }
                    wave.restore_single(actor, t, self.max_hp_restore_ratio* wave.get_max_health(actor));
                }
                // 3rd
