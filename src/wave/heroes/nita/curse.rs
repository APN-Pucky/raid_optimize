...
impl Execute for Curse {
    fn execute(&self, wave : &mut Wave,  skill : &Skill, attacker:InstanceIndex, defender:InstanceIndex, ) {
       ...
       wave.inflict_single(actor, defender, Effect::Poison, self.poison_chance, self.poison_turns);
       wave.restore_single(actor, actor, wave.get_max_health(actor) * self.restore_max_health_per_poison * (wave.effects[defender].get(Effect::Poison) as f32) );
    }
}
