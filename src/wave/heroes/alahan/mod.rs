...
impl Wave<'_> {
    pub fn on_damage_dealt_alahan(&mut self, actor : InstanceIndex, _target :InstanceIndex,dmg : f32,skill : &Skill) {
        if let Skill::SpiritCall(SpiritCall { attack_damage_ratio: _, restore_hp_damage_ratio, remove_all_buffs: _, heal_lowest_ally, increase_hp: _,.. }) = skill  {
        ...
        for i in self.get_ally_indices(actor) {
            if self.is_alive(i) {
                for (si , s) in  self.heroes[i].skills.iter().enumerate() {
                    if let Skill::Commendation(Commendation{max_hp_restore_ratio, attack_up_turns,..  }) = s  {
