...
impl Wave<'_> {
    pub fn refresh_enemy_team<F>(&mut self, actor: InstanceIndex, effect_closure: &F)
    where
        F: Fn(Effect) -> bool,
    {
    ...
    pub fn refresh<F>(&mut self, actor: InstanceIndex, target: InstanceIndex, effect_closure: &F)
    where
        F: Fn(Effect) -> bool,
    {
