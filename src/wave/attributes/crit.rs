    ...
    pub fn get_crit_rate(&self, actor: InstanceIndex) -> f32 {
        ...
        indent!({
            if self.get_faction(actor) == Faction::DragonTribe {
                ...
                let r = 1.0 + xfact * n;
                debug!(
                    "{} has {}*{} bond with DragonTribe -> crit_rate * {}",
                    self.name(actor),
                    n,
                    xfact,
                    r
    ...
    pub fn get_crit_damage(&self, actor: InstanceIndex) -> f32 {
        ...
        indent!({
            ...
            if self.effects[actor].has(Effect::CritDamageUpI) {
                let xfact = 1.25;
                debug!("{} has I -> crit_damage * {}", self.name(actor), xfact);
                fact *= xfact;
            }
            if self.effects[actor].has(Effect::CritDamageUpII) {
                let xfact = 1.5;
                debug!(
                    "{} has CritDamageUpII -> crit_damage * {}",
                    self.name(actor),
                    xfact
                );
                fact *= xfact;
            ...
            if self.effects[actor].has(Effect::CritDamageDownI) {
                let xfact = 0.75;
                debug!(
                    "{} has CritDamageDownI -> crit_damage * {}",
                    self.name(actor),
                    xfact
                );
                fact *= xfact;
            ...
            if self.effects[actor].has(Effect::CritDamageDownII) {
                let xfact = 0.5;
                debug!(
                    "{} has CritDamageDownII -> crit_damage * {}",
                    self.name(actor),
                    xfact
                );
                fact *= xfact;
