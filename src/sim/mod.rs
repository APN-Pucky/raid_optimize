...
impl Sim {
    ...
    pub fn run(&self , tx :UnboundedSender<CombinedResult> )   {
        ...
        vecit.par_iter().map(|_i| {
            ...
            let mut wave = Wave::new(&mut instance, &mut players ,self.args.stats,self.args.turn_limit,self.args.turn_meter_threshold);
            for x in 0..iter {
                cr.add_result(&wave.run());
                wave.reset();
                if (x) % 1000   == 0 || x == iter { // plus one because we start at 0 and want the score added after the iteration
                    match tx.send(cr) {
                        Ok(_) => {},
                        Err(_e) => {
                            return ();
